...
interface AxleConfig {
  type: AxleType;
  label: string;
  dual: boolean; // true = duplo (interno + externo)
}
...
function getAxleConfigs(tipo: string, eixos: number): AxleConfig[] {
  ...
  if (tipo === "Toco" || tipo === "Caminhão 3/4") {
    ...
    // Generic: first direcional already added, rest are dual
...
function generatePositionCodes(axles: AxleConfig[]): string[][] {
  // Each axle returns an array of position codes
  // Convention: D=Direcional, T=Tração, L=Livre, number=axle, E=Esquerdo, D=Direito, I=Interno, X=Externo
  return axles.map((axle, idx) => {
    const num = idx + 1;
    const prefix = axle.type === "direcional" ? "D" : axle.type === "tracao" ? "T" : "L";
    if (axle.dual) {
      // 4 tires: Left External, Left Internal, Right Internal, Right External
      return [
        `${prefix}${num}EE`, // Esquerdo Externo
        `${prefix}${num}EI`, // Esquerdo Interno
        `${prefix}${num}DI`, // Direito Interno
        `${prefix}${num}DE`, // Direito Externo
      ];
    }
    // 2 tires: Left, Right
...
function getSulcoColor(sulcoAtual: number | null, sulcoInicial: number | null): string {
  if (sulcoAtual == null || sulcoInicial == null || sulcoInicial === 0) return "bg-muted text-muted-foreground";
  const pct = (sulcoAtual / sulcoInicial) * 100;
...
function getPressaoColor(pressaoAtual: number | null, pressaoIdeal: number | null): string {
  if (pressaoAtual == null || pressaoIdeal == null || pressaoIdeal === 0) return "text-muted-foreground";
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
          <span className="text-[9px] font-mono font-bold truncate w-full text-center">{tire.id_unico}</span>
          {/* Sulco bar */}
  ...
  return (
    <div className="flex flex-col items-center gap-2 py-4">
          ...
          <div className="mt-4 flex flex-col items-center gap-1">
            ...
            <div className="flex gap-2">
              {Array.from({ length: quantidadeEstepes }).map((_, i) => {
                ...
                return (
                  <TireSlot key={code} positionCode={code} tire={tire} side="left" />
                );
              })}
            </div>
          </div>
      ...
      <div className="flex gap-3 mt-2 text-xs text-muted-foreground">
        ...
        <span>•</span>
        <span>{positionGrid.flat().length + (possuiEstepe ? quantidadeEstepes : 0)} posição(ões)</span>
      </div>
    </div>
  );
}
